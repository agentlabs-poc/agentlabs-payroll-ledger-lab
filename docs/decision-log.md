...
| PAY-PROCESS-010 | User direction; current gate | Complete and review the handbook before continuing implementation | User questioned moving into implementation before handbook completion and then directed “lets fix the hanbook first”. Preserve local code without treating it as accepted specification; reconcile the hub/current Core baseline, settle material ambiguity and finish the handbook review first. |
...
| PAY-Q-020 | Closed as superseded by PAY-ARCH-006 | Must approval withdrawal retain the same draft or require cancellation/rebuilding? | The user clarified that this is organizational Layer-2 policy, then requested closure as superseded. Neither alternative is a universal Layer-1 rule. [Closure and rationale](gap-closure-work.md#pay-q-020--withdrawing-approval-without-changing-draft-amounts). |
...
| PAY-CORE-002 | Agreed with expiry amendment under PAY-Q-004 | Salary earnings record earning entitlement; monthly standing instructions record recurring payroll directions with an effective lifetime and expiry, producing earnings or deductions while applicable. | User's five-month loan example establishes finite applicability while loan business logic remains outside the payroll core. [Rationale, examples, counterexamples, and open mechanics](core-concepts.md#pay-core-002--salary-earnings-and-monthly-instructions). Expiry representation is engineering work; skipped-installment policy belongs upstream. Source expire/replace and the exclusion of business-overlap inference were subsequently settled under PAY-SOURCE-001 and PAY-CORE-008. Implementation gap PAY-GAP-001 is recorded. |
| PAY-CORE-003 | Agreed under PAY-Q-005 | Consume one-time instructions at commit, retaining the committed reference; abandoned drafts do not consume them, and later ordinary payrolls cannot reuse consumed instructions. | User answered "approved" after clarification. [Rationale and example](core-concepts.md#pay-core-003--when-a-one-time-instruction-is-consumed): avoid losing an instruction through an abandoned draft or applying its effect twice. Partial application/restoration are not adopted. Later corrections do not automatically restore consumption; reservation mechanics are an implementation choice. PAY-GAP-002 records missing cross-draft enforcement. |
| PAY-CORE-004 | Agreed under PAY-Q-006 | A monthly standing instruction has one ordinary committed application per employee and applicable payroll month, retaining eligibility in later months until expiry. | User answered "approved". [Rationale and example](core-concepts.md#pay-core-004--monthly-application-versus-instruction-lifetime): avoid duplicate monthly effects without exhausting the whole recurring instruction. Version/application mapping is engineering work; splits and skipped-installment policy are not adopted. Subsequent-month corrections preserve prior applications under PAY-CORE-011. PAY-GAP-003 records missing enforcement. |
...
| PAY-ARCH-006 | User-confirmed boundary; supersedes PAY-Q-020 and universal workflow placement | Layer 1 protects payroll records and operations; Layer 2 selects organizational approval, hold, batch-selection and source-reconciliation policy | User explained that an upper layer may reconcile before commit or treat the draft as final, then requested rationale and superseded closure. [Boundary, examples and consequences](payroll-policy-boundary.md). Earlier mandatory reconciliation remains a policy example; immutable money/history and authorized operations remain core. |
...
PAY-ARCH-006 is the current interpretation boundary for earlier reconciliation
and approval requirements. Preserve their historical rationale without treating
one organization’s workflow as a universal Layer-1 requirement.
...
|---|---|---|---|
| PAY-CORE-017 | Superseded as a core proposal; PAY-Q-020 closed | Retain an unchanged draft after approval withdrawal and require fresh approval | An optional organizational policy, not a mandated core transition. [Layer-1/Layer-2 rationale](payroll-policy-boundary.md) preserves both alternatives. |
